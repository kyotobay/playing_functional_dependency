...
//Class brick must become struct - Let's try!
//A struct cannot contain functions, right? Yes!
...
struct brick
{
	...
	brick(int vars);  //constructor? Anywayz, the fuck out of the struct!
	void read_brick(); //the fuck

	int get_num_vars() {return num_vars;} //the fuck
	int *get_var_list() {return var_list;} //the fuck
	bool *get_table() {return table;}  //the fuck

    	void fill_rectangle(rectangle_entry *rectangle, int count); //the fuck
};
...
//--------------------------------------------------------------------------
// These 3 functions are commented out, because they are part of the class in c++, so they have to be changed!
